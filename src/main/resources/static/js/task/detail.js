...
　　$('#btn-addProject').click(function(event) {
		//帰社ボタン更新
		addProject();
	});
...
function addLine() {
	...
	var formData = $('#task-detail-form').serializeArray();
	console.log(formData);

	 // ajax通信
  $.ajax({
    type : "GET",
    url : '/task/detail/get/list',
    data: formData,
    dataType : 'json',
...
function createDataTables() {
  ...
  // DataTables作成
  table = $('#task-detail-table').DataTable({
    // 日本語化
    language: {
    ...
    //データと列のマッピング
    columns: [
      { data: 'projectId'}, // プロジェクトID
      //{ data: 'projectName'}, // 削除
      { data: 'projectName'}, // プロジェクト名
      { data: 'taskProcess'}, // 誕生日
